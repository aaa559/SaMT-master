...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch=None, oom_batch=None):
        ...
        if self.cuda:
            self.criterion[0]= self.criterion[0].cuda()
            self.criterion[1] = self.criterion[1].cuda()
    ...
    def load_checkpoint(
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            assert last_optim['criterion_name'] == self.criterion[0].__class__.__name__, \
                'Criterion does not match; please reset the optimizer (--reset-optimizer).'
            assert last_optim['criterion_name'] == self.criterion[1].__class__.__name__, \
                'Criterion does not match; please reset the optimizer (--reset-optimizer).'
    ...
    def train_step(self, samples, sentence_vec,dummy_batch=False, raise_oom=False):
        ...
        self._set_seed()
        self.model.train()
        self.criterion[0].train()
        self.criterion[1].train()
        self.zero_grad()
        ...
        for i, sample in enumerate(samples):
            # sentences_id = sample['id']
            # sentences_id = sentences_id.tolist()
            # sen_vec = []
            # for j in sentences_id:
            #     sen = sentence_vec[j]  # 一个句向量
            #     sen_vec.append(sen)  # 一个batch的句向量
            ...
            try:
                with maybe_no_sync():
                    ...
                    model1 = SentenceTransformer('sentence-transformers/distiluse-base-multilingual-cased-v1')
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self.model, model1,sentence_vec,self.criterion, self.optimizer,
                        ignore_grad
                    )
        ...
        logging_output = self.task.aggregate_logging_outputs(
            logging_outputs, self.criterion[0]
        )
        sample_size = self.task.grad_denom(sample_sizes, self.criterion[0])
    ...
    def valid_step(self, sample,valid_vec, raise_oom=False):
        """Do forward pass in evaluation mode."""
        with torch.no_grad():
            self.model.eval()
            self.criterion[0].eval()
            self.criterion[1].eval()
            # sentences_id = sample['id']
            # sentences_id = sentences_id.tolist()
            # val_vec = []
            # for i in sentences_id:
            #     sen = valid_vec[i]  # 一个句向量
            #     val_vec.append(sen)  # 一个batch的句向量
            sample = self._prepare_sample(sample)
            if sample is None:
                sample = self._prepare_sample(self._dummy_batch)
                ignore_results = True
            ...
            try:
                ...
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, model1,self.criterion,valid_vec
                )
            except RuntimeError as e:
        ...
        # aggregate logging outputs and sample sizes
        logging_output = self.task.aggregate_logging_outputs(
            logging_output, self.criterion[0]
        )
        sample_size = self.task.grad_denom(
            sample_size, self.criterion[0]
        )
