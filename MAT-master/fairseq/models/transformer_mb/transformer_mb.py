...
class TransformerMBModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # fmt: on
        # fmt: on
        parser.add_argument('--encoder-group-size', type=int, metavar='N',
                            help='each encoder group size')
        parser.add_argument('--decoder-group-size', type=int, metavar='N',
                            help='each decoder group size')
        # parser.add_argument("--encoder-group-type",type=str, metavar="STR",default="sparse")
        # parser.add_argument("--decoder-group-type", type=str, metavar="STR", default="sparse")
        parser.add_argument("--softmax-temperature",type=int,metavar="STR" )
...
class TransformerMBEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        assert args.encoder_group_size <= args.encoder_layers
        #self.encoder_group_type = getattr(args, "encoder_group_type", "sprase")
        self.encoder_group_size = args.encoder_group_size
        self.encoder_group_number = args.encoder_layers // args.encoder_group_size
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        if self.join_layer:
            ...
            x = torch.stack([encoder_states[i] for i in
                             range(self.encoder_group_size - 1, len(encoder_states), self.encoder_group_size)],
                            dim=0)
...
class TransformerMBDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.decoder_group_size = args.decoder_group_size
        self.decoder_group_number = args.decoder_layers // args.decoder_group_size
    ...
    def extract_features(self, prev_output_tokens, encoder_out=None, incremental_state=None, **unused):
        ...
        if self.join_layer:
            ...
            x = torch.stack([decoder_states[i] for i in
                             range(self.decoder_group_size - 1, len(decoder_states), self.decoder_group_size)])
